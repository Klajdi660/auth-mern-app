...
const userRegister = async (firstName, lastName, email, username, password, passwordConfirm) => {
    try {
        ...
        if (selectResult.length > 0) {
            ...
            if (existingUser.email === email) {
                return { 
                    status: 400, 
                    error: true, 
                    message: `User with this email "${email}" already exists. Please choose another email` 
                };
            } else if (existingUser.username === username) {
                return { 
                    status: 400, 
                    error: true, 
                    message: `User with this username "${username}" already exists. Please choose another username` 
                };
            }
        ...
        return { 
            status: 201, 
            error: false, 
            message: "A message with a weblink has been sent to your email address. Please click that link to proceed." 
        };
    } catch (error) {
        return { 
            status: 401, 
